...
+ (ZHVersionManager *)versionDetection {
    return [[ZHVersionManager alloc] init];
}

/// 版本检测
- (void)requestVersionUpdateQueryWithAppleId:(NSString *)appleId promptType:(WYVersionPromptType)promptType cycle:(NSInteger)cycle isShowUpdateContent:(BOOL)isShowUpdateContent{
    self.promptType = promptType;
    self.cycle = cycle;
    self.isShowUpdateContent = isShowUpdateContent;
             ...
             switch (self.promptType) {
                 case WYVersionPromptNomal:
                 {
                     ...
                     [self showAlertWithTitle:@"应用版本有更新"
                      ...
                      {
                          if (btnIndex == 1) {
                              [[UIApplication sharedApplication] openURL:[NSURL URLWithString:trackViewUrl] options:0 completionHandler:nil];
                          }
